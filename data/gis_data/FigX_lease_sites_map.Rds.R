...
# Directories
gisdir <- "~/Dropbox/Chris/UCSB/projects/mpa_projects/boem/data/gis_data/processed"
...
# Directory
datadir <- "~/Dropbox/Chris/UCSB/consulting/halibut_bycatch/data/landings_receipts/processed"

# Read data
data_orig <- readRDS(file=file.path(datadir, "CDFW_2000_2020_landings_receipts.Rds"))
...
data_spp <- data_orig %>% 
  # Summarize
  group_by(year, block_id, species) %>% 
  summarize(landings_lb=sum(landings_lb, na.rm=T)) %>% 
  ungroup() %>% 
...
# Plot data
g <- ggplot(data_spp, aes(x=landings_mt, y=reorder(species, landings_mt))) +
